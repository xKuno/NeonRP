		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					print(v.modType .. ' - tuning type')
					print(source)
					if isRimMod then
						price = math.floor(vehiclePrice * data.current.price / 100)
						TriggerServerEvent("esx_gscustom:buyMod", price)
						TriggerServerEvent('ilukaszef:logmod', vehiclePrice, vehicleData, GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)), price)
	...
	while true do
		...
		if (PlayerData.job ~= nil and PlayerData.job.name == 'mecano') or Config.IsmecanoJobOnly == false then
			if IsPedInAnyVehicle(playerPed, false) then
				...
				if IsControlJustReleased(0, 182) and isInLSMarker then
...
RegisterNetEvent('LSC:EasyAdmin')
AddEventHandler('LSC:EasyAdmin', function()
	GetAction({value = 'main'})
end)
