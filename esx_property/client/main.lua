...
function CreateBlips()
	for i=1, #Config.Properties, 1 do
		...
		if property.entering then
			...
			SetBlipColour (Blips[property.name], 4)
...
function SetPropertyOwned(name, owned)
	...
	if owned then
		...
		if not found then
			...
			SetBlipColour (Blips[enteringName], 3)
			SetBlipAsShortRange(Blips[enteringName], true)
...
function OpenPropertyMenu(property)
	...
	if PropertyIsOwned(property) then
		...
		if not Config.EnablePlayerManagement then
			-- table.insert(elements, {label = _U('buy') .. " - Koszt: $".. property.price ..'', value = 'buy'})
			table.insert(elements, {label = _U('rent') .. " - Koszt: $" .. math.floor(property.price / 200) ..'', value = 'rent'})
		end
...
function OpenGatewayAvailablePropertiesMenu(property)
	...
	for i=1, #gatewayProperties, 1 do
		if not PropertyIsOwned(gatewayProperties[i]) then
			table.insert(elements, {
				label = gatewayProperties[i].label .. '</span> | Wynajem: <span style="color:green;">$' .. ESX.Math.GroupDigits(gatewayProperties[i].price / 200)..'</span>',
				value = gatewayProperties[i].name,
...
function OpenRoomInventoryMenu(property, owner)
	...
	ESX.ShowAdvancedNotification('Mieszkanie', 'Otwierasz szafke', 'Musisz poczekać chwile', 'CHAR_BLOCKED', 8)
	Draw3DText(plyPos.x, plyPos.y, plyPos.z + 0.2, tostring("~w~Otwieranie szafki"))
	Wait(1500)
		...
		for i=1, #Config.Properties, 1 do
			...
			if property.entering and not property.disabled then
				...
				if distance < 15 then
					...
					local PozycjaWejscia = {
						["x"] = property.entering.x,
						["y"] = property.entering.y,
						["z"] = property.entering.z + 0.8
					}
					ESX.Game.Utils.DrawText3D(PozycjaWejscia, "NACIŚNIJ [~g~E~s~] ABY OTWORZYĆ MENU", 0.55, 1.5, "~b~POSIADŁOŚĆ", 0.7)
					letSleep = false
				end
			...
			if property.exit and not property.disabled then
				...
				if distance < 10 then
					...
					local PozycjaTekstu = {
						["x"] = property.exit.x,
						["y"] = property.exit.y,
						["z"] = property.exit.z + 0.8
					}
					ESX.Game.Utils.DrawText3D(PozycjaTekstu, "NACIŚNIJ [~g~E~s~] ABY WYJŚĆ", 0.55, 1.5, "~b~WYJŚCIE", 0.7)
					letSleep = false
				end
			...
			if property.roomMenu and hasChest and not property.disabled then
				...
				if distance < 8 then
					...
					local PozycjaTekstu = {
						["x"] = property.roomMenu.x,
						["y"] = property.roomMenu.y,
						["z"] = property.roomMenu.z + 0.8
					}
					ESX.Game.Utils.DrawText3D(PozycjaTekstu, "NACIŚNIJ [~g~E~s~] ABY OTWORZYĆ SZAFĘ", 0.55, 1.5, "~b~SZAFA", 0.7)
					letSleep = false
				end
	...
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'room_menu' and not exports['esx_policejob']:jestzakuty() then
