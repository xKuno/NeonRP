...
AddEventHandler('RealisticVehicleFailure:engine', function(health)
	healthEngineLast = health
	healthEngineCurrent = health
	healthEngineNew = health
end)

AddEventHandler('RealisticVehicleFailure:body', function(health)
	healthBodyLast = health
	healthBodyCurrent = health
	healthBodyNew = health
end)

local function isPedDrivingAVehicle(ped)
...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle(ped) then
			...
			healthEngineDeltaScaled = healthEngineDelta * cfg.damageFactorEngine * cfg.classDamageMultiplier[vehicleClass + 1]
			...
			healthBodyDeltaScaled = healthBodyDelta * cfg.damageFactorBody * cfg.classDamageMultiplier[vehicleClass + 1] * (cfg.specialDamageMultiplier[GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))] or 1.0)
			...
			healthPetrolTankDeltaScaled = healthPetrolTankDelta * cfg.damageFactorPetrolTank * cfg.classDamageMultiplier[vehicleClass + 1]
			...
			if healthEngineCurrent <= cfg.engineSafeGuard+1 and cfg.limpMode == false then
				if IsVehicleEngineOn(vehicle) then
					TriggerServerEvent("wyspa_garage:forceUpdateOwnedVehicle", ESX.Game.GetVehicleProperties(vehicle))
					ESX.ShowNotification("Zbyt duże uszkodzenia, dalsza jazda niemożliwa!")
				end
			...
			if pedInSameVehicleLast == true then
				...
				SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fCollisionDamageMult', cfg.collisionMultiplier)

				--Get the EngineDamageMultiplier
				--fEngineDamageMult = GetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fEngineDamageMult')
				--Modify it by pulling all number a towards 1.0
				--local newFEngineDamageMult = 2.50000 ^ cfg.engineDamageExponent	-- Pull the handling file value closer to 1

				SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fEngineDamageMult', cfg.engineMultilpier)
			...
			if pedInSameVehicleLast == true then
				...
				SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fCollisionDamageMult', cfg.collisionMultiplier) -- Restore the original CollisionDamageMultiplier
				SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fEngineDamageMult', cfg.engineMultilpier) -- Restore the original EngineDamageMultiplier
			end
