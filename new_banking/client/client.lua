...
local atms = {
	...
	{name="ATM", id=277, x=-672.3647, y=334.5276, z=82.1723}, --- szpital
	...
	{name="ATM", id=277, x=-1391.02, y=-590.39, z=30.32},
	...
	{name="ATM", id=277, x=-1285.79, y=-572.61, z=30.57},
	...
	{name="ATM", id=277, x=126.65, y=-146.01, z=53.91},
}
	...
	while true do
		...
		for k,v in pairs(atms) do
			...
			if distance <= 1.0 then
				isInMarker, currentZone = true, "atm"
			end
		end
		...
		for k,v in pairs(banks) do
			...
			if distance <= 1.0 then
				isInMarker, currentZone = true, "bank"
			end
		end

		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			HasAlreadyEnteredMarker, LastZone = true, currentZone
			TriggerEvent('new_banking:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('new_banking:hasExitedMarker', LastZone)
		end
...
AddEventHandler('new_banking:hasEnteredMarker', function(zone)
	CurrentAction     = 'open_bank'
	CurrentActionMsg  = "Naciśnij ~INPUT_CONTEXT~ aby użyć karty kredytowej"
	CurrentActionData = {}
end)

AddEventHandler('new_banking:hasExitedMarker', function(zone)
...
AddEventHandler('currentbalance1', function(balance, playerName)
	local id = PlayerId()
...
-- print(GetDisplayNameFromVehicleModel(-2097817357)) -- punto -361155694