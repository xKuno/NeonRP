...
local blips = {	
	{title="Warsztat", colour=5, id=446, x = 823.69,  y = -893.17,  z = 24.84},
}
...
AddEventHandler("esx:playerLoaded", function(response)
	PlayerData = response
	TriggerServerEvent('duty:zdejscieLSCM')
		...
		if data.current.value == 'vehicle_list' then
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					...
					{label = _U('SlamVan'), value = 'slamvan3'},
					{label = _U('wozek'), value = 'forklift'}	
				}
...
RegisterNetEvent('esx_mecanojob:onFixkit2')
