        ...
        if(IsControlJustPressed(1, Config.key))then
            ...
            if(localVehId and localVehId ~= 0)then
                ...
                if localVehPlateTest ~= nil then
                        ...
                        if(string.lower(plate) == localVehPlate)then
                            ...
                            if(vehicle ~= "locked")then
                                ...
                                if(time > timer)then
                                    ...
                                    TriggerEvent("FeedM:showAdvancedNotification", "NeonRP", "Pojazd", ecosa, 1000, warning)
                                end
                            else
                                TriggerEvent("FeedM:showAdvancedNotification", "NeonRP", "Pojazd", "Kluczy nie ma w środku", 1000, warning)
                            end
                        end
                    end
                    ...
                    TriggerEvent("FeedM:showAdvancedNotification", "NeonRP", "Pojazd", "Nie można zczytać tablic rejestracyjnych", 1000, warning)
                end
            end
        end
    end
...
AddEventHandler("ls:getHasOwner", function(hasOwner, localVehId, localVehPlate, localVehLockStatus)
    if(not hasOwner)then
       ...
       local ecosa = getRandomMsg()
        -- TriggerEvent("ls:notify", getRandomMsg())
        TriggerEvent("FeedM:showAdvancedNotification", "NeonRP", "Pojazd", ecosa, 1000, warning)
    else
        -- TriggerEvent("ls:notify", _U("vehicle_not_owned"))
        TriggerEvent("FeedM:showAdvancedNotification", "NeonRP", "Pojazd", "Ten pojazd nie jest twój", 1000, warning)
    end
end)
...
AddEventHandler('InteractSound_CL:PlayWithinDistance', function(playerNetId, maxDistance, soundFile, soundVolume)
    ...
    local pid = GetPlayerFromServerId(playerNetId)
    if pid ~= -1 then		
...
function Notify(text, duration)
	if(Config.notification)then
		if(Config.notification == 1)then
			...
			Citizen.InvokeNative(0x1E6611149DB3DB6B, true, 1, "NeonRP", "Pojazd", duration)
