...
Citizen.CreateThread(function()
	AddTextEntry("ParaTattoos", "Studio tatuażu")
...
function RemoveTattoo(name, label)
	...
	ESX.ShowNotification("Usunąłeś ~y~" .. GetLabelText(label) .. "~s~ tatuaż")
end

function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "Przejrzeć ofertę tatuatora", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Zmień perspektywe", 21, "Obrócić się", {90, 89}, "Kup/Usuń tatuaż", 191)
		scaleString = sType
	end
end

Citizen.CreateThread(function()
	JayMenu.CreateMenu("tattoo", "Studio tatuażu", function()
        return CloseTattooShop()
    end)
    JayMenu.SetSubTitle('tattoo', "Kategoria")
