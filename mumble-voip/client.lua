		...
		if initialised then
			...
			if mumbleConfig.radioEnabled then
				if not mumbleConfig.controls.radio.pressed then
					if IsControlJustPressed(0, mumbleConfig.controls.radio.key) and not IsPedSwimming(PlayerPedId()) and not IsPedSwimmingUnderWater(PlayerPedId()) then
        ...
        local playersTalking = {'empty'}
			...
			for _, i in ipairs(GetActivePlayers()) do
                if NetworkIsPlayerTalking(i) then
                    playersTalking[count] = "ID: "..GetPlayerServerId(i)
                    count = count + 1
                end
            end
            if playersTalking[1] ~= "empty" then
                ...
                for k,v in pairs(playersTalking) do
                    displayText("~p~" .. v, 0, 255, 255, 255, 255, 0.98, 0.955 - (0.014*(count)))
                    count = count + 1
                end
            end
    end
